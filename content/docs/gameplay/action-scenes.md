...
In action scenes, the GM first establishes which sides are participating in the scene (usually just the players and their enemies, but there could be a third side as well) and which side stars. Each side takes one turn at a time. When it’s the players’ side’s turn, one member of that side can take the following actions:
- Move from one place to another
- Make one roll
- Make a second roll (requires that you spend a total of four drive and/or stress)
...
- A difficulty, ranging from 6 to 30
- An HP value, ranging from 1 to around 10
- A list of tags, each with a star value ranging from 1-5
- A positive condition slot and negative condition slot (starts empty)

For example, the following is a mid-level enemy:
